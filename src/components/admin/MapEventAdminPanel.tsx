...
function formatEventTimeBadge(endAt: string, timeIcon = "⏰", timeFormat = "D_DAY_TIME"): string | null {
  ...
  const icon = timeIcon?.trim() ? `${timeIcon.trim()} ` : "";
...
export default function NaverMapPartnersView({
  ...
  const latestStampLabelRef = useRef(effectiveStampLabel);
  useEffect(() => {
    latestStampLabelRef.current = effectiveStampLabel;
  }, [effectiveStampLabel]);
  ...
  useEffect(() => {
    ...
    function openMiniCard(partner: NaverMapPartnerMarker, position: naver.maps.LatLng) {
      ...
      const currentStampLabel = latestStampLabelRef.current || "버닝찍기";
      ...
      const card = createPartnerMapMiniCardElement(partner, () => {
        ...
        onFavoriteToggle: favoritesEnabledRef.current
          ...
          : undefined,
        stamp: activeStampAction?.enabled || Boolean(activeDbEvent)
          ? {
              visible: true,
